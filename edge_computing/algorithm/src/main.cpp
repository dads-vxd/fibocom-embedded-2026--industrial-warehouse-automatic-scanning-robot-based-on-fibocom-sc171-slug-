...
int main(int, char **) {
  ...
  if (!cfg.wsUrl.empty())
    uploadMgr.start(cfg.wsUrl);

  ControlClient controlClient;
  if (!cfg.controlWsUrl.empty())
    controlClient.start(cfg.controlWsUrl);

  SerialSender serialSender;
  serialSender.setOnReceived([&controlClient](const std::string &line) {
    if (line == "25")
      controlClient.sendLog("发现障碍物");
  });
  controlClient.setCommandCallback(
      [&serialSender, serialEnabled = cfg.serialEnabled](
  ...
  std::chrono::steady_clock::time_point timeSent1;
  bool sent2 = false;
  if (cfg.serialEnabled) {
    serialSender.start(cfg.serialPort, cfg.serialBaudRate);
    serialSender.send("1");
    timeSent1 = std::chrono::steady_clock::now();
  }

  VideoUploader videoUploader;
  if (!cfg.videoUploadUrl.empty())
    videoUploader.start(cfg.videoUploadUrl);
  ...
  for (size_t i = 0; i < cfg.cameras.size(); ++i) {
    ...
    if (!cams.back().isOpen())
      cams.pop_back();
    else if (!cfg.videoUploadUrl.empty())
      cams.back().recorder.setOnSegmentDone(
          [&videoUploader](const std::string &path) {
            videoUploader.enqueue(path);
          });
  }
  ...
  while (true) {
    ...
    if (!cfg.wsUrl.empty() && !barcodeDataMap.empty())
      enqueueMatchedBarcodes(cam.id, tracks, dets, crops, cam.barcodeCache,
                             barcodeDataMap, uploadMgr);
    ...
    if (cfg.serialEnabled) {
      // 15-second timeout: force-send "2" if not sent yet since "1"
      if (!sent2) {
        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
                           std::chrono::steady_clock::now() - timeSent1)
                           .count();
        if (elapsed >= 15) {
          serialSender.send("2");
          sent2 = true;
          std::cout << "[Serial] Timeout: force-sent '2' after " << elapsed
                    << "s" << std::endl;
        }
      }
